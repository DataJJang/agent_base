...
| 공공 배포/형상관리 프로필 | `deliveryDevopsProfile` | 형상관리, CI, artifact, 반입/배포 실행, 승인, smoke/rollback owner | 조건부 필수 |
...
- `web-app`
  ...
  - `deliveryDevopsProfile`은 기본적으로 `Git + Jenkins`와 artifact/반입 모델을 같이 채운다
- `backend-service`
  - `Java 17 + Maven + eGovFrame 4.3 REST + MyBatis`
  - `deliveryDevopsProfile`은 기본적으로 `Git + Jenkins + war + Nexus + manual-was-deploy`
- `batch-worker`
  - `Java 17 + Maven + eGovFrame 4.3 Batch + MyBatis`
  - `deliveryDevopsProfile`은 기본적으로 `Git + Jenkins + jar + Nexus + pipeline-to-vm`
...
- `deliveryDevopsProfile` 같은 공공 배포/형상관리 의사결정 필드
